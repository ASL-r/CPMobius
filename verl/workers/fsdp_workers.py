...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        self._is_ref = self.role in ["ref", "actor_rollout_ref", "proposer_ref"]
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0):
        ...
        self.checkpoint_manager.save_checkpoint(
            ...
            remove_previous_ckpt=self.config.actor.get("remove_previous_ckpt", True),
        )
...
class RewardModelWorker(Worker):
    ...
    def __init__(self, config):
        ...
        self.sigmoid_coef = float(self.config.get("sigmoid_coef", 1.0))
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if config.model.get("is_sequence_level", False):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.config.model.get("is_qwen_prm", False):
                    ...
                    def make_step_rewards(logits, token_masks):
                        ...
                        for i in range(probabilities.size(0)):
                            ...
                            baseline = self.config.model.get("baseline", 0)
                            non_zero_elements_list = [
                                x - baseline for x in non_zero_elements_list
                            ]
                            if len(non_zero_elements_list) == 0:
                                all_scores_res.append(-1)
                            else:
                                all_scores_res.append(min(non_zero_elements_list))
                        return torch.tensor(all_scores_res, device=logits.device)

                    step_sep_id = self.tokenizer.encode("<extra_0>")[0]
                    token_masks = input_ids == step_sep_id
                    rm_score = make_step_rewards(output.logits, token_masks)
                else:
                    rm_score = output.logits  # (batch_size, seq_len, 1)
                    rm_score = rm_score.squeeze(-1)

            if self.config.model.get("is_token_level", True):
                # extract the result of the last valid token
                eos_mask_idx = torch.argmax(
                    position_ids * attention_mask, dim=-1
                )  # (bsz,)
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            if self.config.get("exclude_think", False):
                import re

                match = re.search(r"</think>(.*)", response, re.DOTALL)
                if match:
                    response = match.group(1).lstrip()
            if self.config.model.get("is_qwen_prm", False):
                response = "<extra_0>".join(response.split("\n\n")) + "<extra_0>"
    ...
    def _check_format(self, rm_scores, input_ids):
        # TODO: make the format and the penalty configurable
        format_text = self.config.format_text
        format_penalty = self.config.format_penalty_value
        for i, input_id in enumerate(input_ids):
            text = self.tokenizer.decode(input_id)
            if format_text not in text:
                rm_scores[i] = format_penalty
        return rm_scores
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            for micro_batch in micro_batches:
                ...
                if self.config.get("format_penalty", False):
                    rm_score = self._check_format(rm_score, micro_batch["input_ids"])
                output.append(rm_score)
            scores = torch.cat(output, dim=0)  # (batch_size)

            if self.apply_sigmoid:
                scores = torch.sigmoid(self.sigmoid_coef * scores)
