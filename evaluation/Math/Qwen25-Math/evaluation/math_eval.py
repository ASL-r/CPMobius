...
def parse_args():
    ...
    parser.add_argument(
        "--system_prompt_path",
        default=os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../system_prompt.md")),
        type=str,
    )
    parser.add_argument(
        "--apply_chat_template",
...
def prepare_data(data_name, args):
    ...
    print(f"Entered pdb in PID {os.getpid()}, RANK = {os.environ.get('RANK', 'N/A')}")
...
def main(llm, tokenizer, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        if epoch == 0:
            ...
            print(prompts[0])
        print(
            f"temp={args.temperature}, topp={args.top_p}, repp={args.repetition_penalty}"
        )
        if args.use_vllm:
